...
Public Class FrmListaBajasActivo
    ...
    Private restriccionReversar As Restriccion = Nothing
...
#Region "New"
    Public Sub New(ByVal _Sistema As Sistema, ByVal _Restriccion As Restriccion, ByVal _busqueda As Boolean, Optional ByVal _OpcionNuevo As Enumerados.EnumOpciones = Enumerados.EnumOpciones.ListadoActivos)
        ...
        restriccionReversar = _Sistema.Restricciones.Buscar(Enumerados.EnumOpciones.DarBajaActivoReversar)
        If restriccionReversar.Restri_Ingreso Then
            btnReversarBaja.Enabled = True
        Else
            btnReversarBaja.Enabled = False
        End If
...
#End Region
    ...
    Private Sub btnLlenar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLlenar.Click
        Auditoria.Registrar_Auditoria(Restriccion, Enumerados.enumTipoAccion.Impresion, "Realizó búsqueda de bienes dados de baja.")
        Llenar_datos()
    End Sub
    ...
    Private Sub btngenerartxt_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btngenerartxt.Click
        ...
        If bReturn AndAlso Not ds Is Nothing AndAlso ds.Rows.Count > 0 Then
            ...
            Auditoria.Registrar_Auditoria(Restriccion, Enumerados.enumTipoAccion.Adicion, "Generó txt.")

        End If
    End Sub

    Private Sub btnReversarBaja_Click(sender As System.Object, e As System.EventArgs) Handles btnReversarBaja.Click
        Dim codigo As String = ""
        Dim _activo As Activo

        codigo = InputBox("Escriba el código de barras que desea reversar la baja:", "Reversar baja")
        If Not String.IsNullOrWhiteSpace(codigo) Then
            _activo = New Activo(Sistema.OperadorDatos, codigo)
            If _activo.RecargarporCodigoBarra() Then
                If Not _activo.Activo Then
                    If MsgBox("¿Está seguro que desea reversar la baja de este bien? " + _activo.Activo_CodigoBarra + " " + _activo.Descripcion, MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                        If _activo.ReversarBaja() Then
                            Auditoria.Registrar_Auditoria(restriccionReversar, Enumerados.enumTipoAccion.Adicion, "(" + _activo.Activo_CodigoBarra + ") Se reversó la baja de " + _activo.Descripcion)
                            MsgBox("Baja reversada con exito.")
                            Llenar_datos()
                        Else
                            Auditoria.Registrar_Auditoria(restriccionReversar, Enumerados.enumTipoAccion.Adicion, "(" + _activo.Activo_CodigoBarra + ") Ocurrió un error durante la reversión de la baja de " + _activo.Descripcion)
                            MsgBox("No se reversó la baja.")
                        End If
                    End If
                Else
                    Auditoria.Registrar_Auditoria(restriccionReversar, Enumerados.enumTipoAccion.Adicion, "(" + _activo.Activo_CodigoBarra + ") Intentó reversar la baja de " + _activo.Descripcion + " pero no se encuentra dado de baja.")
                    MsgBox("El bien " + _activo.Activo_CodigoBarra + " " + _activo.Activo_Descripcion + " no se encuentra dado de baja.")
                End If
                'Else
                'MsgBox("No recargo")
            End If
